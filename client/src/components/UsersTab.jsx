...
const UsersTab = () => {
  ...
  useEffect(() => {
    ...
    if (userRemoval) {
      toast({ description: 'Utilizatorul a fost șters.', status: 'success', isClosable: true });
    }
              ...
              <Tbody>
                {userList &&
                  userList.map((user) => (
                    <Tr key={user._id}>
                      <Td>
                        {user.name} {user._id === userInfo._id ? '(Tu)' : ''}
                      </Td>
                      <Td>{user.email}</Td>
                      <Td>{new Date(user.createdAt).toDateString()}</Td>
                      <Td>{user.isAdmin === 'true' ? <CheckCircleIcon color='purple.500' /> : ''}</Td>
                      <Td>
                        <Button
